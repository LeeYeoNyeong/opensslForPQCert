...
# env handed to both orchestrator roles (matrix defaults come from the script)
ORCH_ENV="RUNS=$RUNS PORT=$PORT RESUME=$RESUME"
[ -n "${FORMATS:-}" ] && ORCH_ENV="$ORCH_ENV FORMATS='$FORMATS'"
[ -n "${ALGS:-}" ]    && ORCH_ENV="$ORCH_ENV ALGS='$ALGS'"
[ -n "${LOSSES:-}" ]  && ORCH_ENV="$ORCH_ENV LOSSES='$LOSSES'"
[ -n "${BWS:-}" ]     && ORCH_ENV="$ORCH_ENV BWS='$BWS'"
...
run_pair() { # $1=pair  (one background job per pair)
    local pair="$1"
    local sreg sip cip label lf
    ...
    lf="$LOGDIR/pair_$pair.log"

    {
        echo "=== pair $pair : client($cip) -> server($sip in $sreg), RUNS=$RUNS RESUME=$RESUME ==="
        ...
        local crc=0
        ssh_to "$cip" "cd $REMOTE_HYBRID && sudo -E $ORCH_ENV IFACE=$IFACE \
        ...
        echo "=== pair $pair finished (client rc=$crc, server rc=$src) ==="
        [ "$crc" = 0 ] && [ "$src" = 0 ] || exit 1
    } >"$lf" 2>&1
}
...
PIDS=(); NAMES=()
for pair in $SEL; do
    log "launching pair $pair (log: run_logs/pair_$pair.log)"
    run_pair "$pair" &
    PIDS+=("$!"); NAMES+=("$pair")
done

log "all pairs running in parallel; tail -f run_logs/pair_*.log to follow"
RC=0
for i in "${!PIDS[@]}"; do
